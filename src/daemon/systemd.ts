 ...
 */
function parseSystemctlFailure(detail: string): { reason: string; fix: string } {
  ...
  if (d.includes("failed to connect to bus")) {
    return {
      ...
      fix: [
        "请启用用户 linger: sudo loginctl enable-linger $USER",
        "然后刷新会话: systemctl --user daemon-reexec",
      ].join("\n"),
    };
  }
...
async function assertSystemdAvailable() {
  ...
  const { reason, fix } = parseSystemctlFailure(res.stderr || res.stdout);
  throw new Error([`systemctl --user unavailable: ${reason}`, `修复建议: ${fix}`].join("\n"));
}
