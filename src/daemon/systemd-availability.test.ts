...
import { isSystemdUserServiceAvailable, isSystemdUserServiceAvailableDetailed } from "./systemd.js";
import {
...
describe("isSystemdUserServiceAvailableDetailed", () => {
  ...
  it("returns available=false with WSL fix for not-booted-with-systemd error", async () => {
    execFileMock.mockImplementation(
      (_cmd: string, _args: string[], _opts: unknown, cb: Function) => {
        const err = new Error("System has not been booted with systemd") as Error & {
          stderr?: string;
          code?: number;
        };
  ...
  it("returns available=false when systemctl command is not found", async () => {
    execFileMock.mockImplementation(
      (_cmd: string, _args: string[], _opts: unknown, cb: Function) => {
        const err = new Error("systemctl: command not found") as Error & {
          stderr?: string;
          code?: number;
        };
  ...
  it("returns available=false when systemd user directory is missing", async () => {
    execFileMock.mockImplementation(
      (_cmd: string, _args: string[], _opts: unknown, cb: Function) => {
        const err = new Error("No such file or directory") as Error & {
          stderr?: string;
          code?: number;
        };
  ...
  it("returns available=false with generic fallback for unknown errors", async () => {
    execFileMock.mockImplementation(
      (_cmd: string, _args: string[], _opts: unknown, cb: Function) => {
        const err = new Error("Some obscure systemd error") as Error & {
          stderr?: string;
          code?: number;
        };
...
describe("isWSLEnv", () => {
  ...
  it("returns true when WSL_INTEROP is set", () => {
    setEnv({ WSL_INTEROP: "some/path", WSL_DISTRO_NAME: undefined, WSLENV: undefined });
    expect(isWSLEnv()).toBe(true);
    setEnv({ WSL_INTEROP: undefined });
  });

  it("returns true when WSL_DISTRO_NAME is set", () => {
    setEnv({ WSL_DISTRO_NAME: "Ubuntu", WSL_INTEROP: undefined, WSLENV: undefined });
    expect(isWSLEnv()).toBe(true);
    setEnv({ WSL_DISTRO_NAME: undefined });
  });

  it("returns false when no WSL env vars are set", () => {
    setEnv({ WSL_INTEROP: undefined, WSL_DISTRO_NAME: undefined, WSLENV: undefined });
...
describe("detectLinuxDistro", () => {
  ...
  it("parses /etc/os-release for Ubuntu", async () => {
    fsReadFileMock.mockResolvedValue('ID=ubuntu\nID_LIKE="debian ubuntu"\nVERSION_ID="22.04"\n');
