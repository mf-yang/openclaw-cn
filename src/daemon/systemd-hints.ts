...
/** Test-only: reset the distro cache. */
export function _resetDistroCache(): void {
  _distroCache = null;
}
...
export async function detectLinuxDistro(): Promise<LinuxDistroInfo> {
  ...
  const wsl = !!process.env.WSL_INTEROP || !!process.env.WSL_DISTRO_NAME || !!process.env.WSLENV;
...
export function isWSLEnv(): boolean {
  return !!process.env.WSL_INTEROP || !!process.env.WSL_DISTRO_NAME || !!process.env.WSLENV;
}
...
export function renderSystemdUnavailableHints(
  ...
  if (distro) {
    const isUbuntu =
      distro.id === "ubuntu" || distro.like.includes("ubuntu") || distro.like.includes("debian");
    if (isUbuntu) {
      return [
        "Ubuntu/Debian 用户请确保：",
