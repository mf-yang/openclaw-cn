...
- **修复 Ubuntu/WSL 守护进程安装失败时缺乏针对性提示**（#550，原 #543）：`gateway install` 在 systemd 用户服务不可用的环境（WSL2 未启用 systemd、Ubuntu/Debian 未启用 user linger 等）失败时，错误信息仅返回原始 systemctl 文本，用户难以定位修复路径。修复方案：(1) 新增 `detectLinuxDistro()` 通过读取 `/etc/os-release` 标准识别发行版（含 `ID_LIKE`，覆盖 Pop!\_OS 等衍生发行版）；(2) 新增 `buildInstallErrorMessage()` 统一构造多行错误消息（数组 `.join("\n")` 替代散落 `\n` 拼接），按发行版/WSL 给出具体修复步骤（如 `loginctl enable-linger`、`/etc/wsl.conf [boot] systemd=true`）；(3) `assertSystemdAvailable` 复用首次 `execSystemctl` 结果，避免重复调用；(4) 新增 27 条单元测试覆盖 `isSystemdUserServiceAvailableDetailed`、`detectLinuxDistro`、`renderSystemdUnavailableHints`、`buildInstallErrorMessage`、`isWSLEnv`。感谢 @fanzifeng 🙏
